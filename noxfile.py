...
def codeqa(session: nox.Session):
    ...
    session.run("flake8", "src/antsibull_tool", *session.posargs)
    session.run(
        "pylint",
        "--rcfile",
        ".pylintrc.automated",
...
def typing(session: nox.Session):
    ...
    session.run("mypy", "src/antsibull_tool")
...
def bump(session: nox.Session):
    ...
    if len(session.posargs) > 1:
        ...
        session.run(
            "git",
            "add",
            "src/antsibull_tool/__init__.py",
            str(fragment_file),
            external=True,
        )
...
def publish(session: nox.Session):
    ...
    session.run("git", "add", "src/antsibull_tool/__init__.py", external=True)
...
def install_env(session: nox.Session):
    """
    Install antsibull-tool and the other project in the the local environment.
